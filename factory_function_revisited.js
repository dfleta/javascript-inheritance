...
console.log(r instanceof range);                    // false, no usamos new, sino create
...
console.log(range.methods.isPrototypeOf(s));    // true, range.methods esta en la cadena de prototipos de s
console.log(range.methods.isPrototypeOf(t));    // true
console.log(r.isPrototypeOf(s));                // true

range.methods.action = function() {             // añadir propiedad de manera dinamica en el prototipo inicial
    return "accion en range.methods";
};

console.log(r.action());                        // r y s tambien la tienen
console.log(s.action());

r.step = "integer";         // extendemos dinamicamente r
console.log(s.step);        // JS encuentra step en r meiante la cadena de prototipos

s.step = "decimal";
console.log("step en s es " + s.step);      // cambios solo en s
console.log("step en r es " + s.step);

r.description = {                                // creo una propiedad mutable (objeto litera) en r
    values: "from " + r.from + " to " + r.to,    // como es un literal no puedo usar encapsulacion - this
    step: r.step,
};
console.log(s.description.values + " en " + s.description.step);    // en s existe tambien

s.description.step = s.step;
console.log(s.description.step);        // muto el objeto en s
console.log(r.description.step);        // ha mutado en r, en el prototipo!!

s.description = "en numeros romanos";
console.log(r.description.step);        // sobreescribir la propiedad mutable en s => en el prototipo no cambia
